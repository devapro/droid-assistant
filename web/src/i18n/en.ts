...
export const en = {
  ...
  record: {
    ...
    actionItemsTab: (n: number) => `Action items (${n})`,
  ...
  session: {
    ...
    actionItemLinked: 'That line is already on the Action items list.',
    generate: (kind: string) => `Generate ${kind}`,
    generating: 'Generating…',
    generateHint: (kind: string) => `Nothing has produced ${kind} for this recording yet.`,
    producedNothing: (kind: string) =>
      `${kind.replace(/_/g, ' ')} ran but produced nothing for this recording.`,
    onDemandHint: (kind: string) =>
      `${kind[0]!.toUpperCase()}${kind.slice(1)} is not generated automatically — it costs a call over the whole conversation, so it runs when you ask for it.`,
    pluginUnavailable: (reason: string) => `Unavailable — ${reason}`,
